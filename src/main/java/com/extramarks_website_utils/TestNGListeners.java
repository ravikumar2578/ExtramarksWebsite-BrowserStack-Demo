...
public class TestNGListeners implements IReporter{
	//This is the customize emailabel report template file path.
		private static final String emailableReportTemplateFile = "D:\\Automation\\ExtramarksWebsite\\test-output\\emailable-report.html";
		@Override
		public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
			...
			try
			{
				...
				String customReportTitle = this.getCustomReportTitle("ExtramarksWesiteAutomation - Demo");
		...
		private String getTestMethodReport(String testName, IResultMap testResultMap, boolean passedReault, boolean skippedResult)
		{
			...
			if(skippedResult)
			{
				resultTitle += " - Skipped Test Cases";
				color = "yellow";
			}else
			{
				if(!passedReault)
				{
					resultTitle += " - Failed Test Cases";
					color = "red";
				}else
				{
					resultTitle += " - Passed Test Cases";
					color = "red";
				}
			}
			...
			for(ITestResult testResult : testResultSet)
			{
				...
				testMethodName=testMethodName+" for Valid User";
				//Get startDateStr
