...
public class Xls_Reader {
	 ...
	 * "D:\\testing data2018\\Emscc_data.xlsx";
	 */
	// import statements
	...
	public int getCellRowNumber(String sheetName, String locatorName, String locatorType) {
		int k = 0;
		for (int i = 1; i <= getRowCount(sheetName); i++) {
			if (getCellData(sheetName, 0, i).equalsIgnoreCase(locatorName)) {

				if (getCellData(sheetName, 1, i).equalsIgnoreCase(locatorType)) {
					k = i;
					break;
				}
			}
	...
	public boolean setCellData(String sheetName, String testCaseName, String colName, int rowNum, String data) {
		try {
			fis = new FileInputStream(path);
			workbook = new XSSFWorkbook(fis);
			...
			row = sheet.getRow(colStartRowNum - 1);
			for (int i = 0; i < row.getLastCellNum(); i++) {
				// System.out.println(row.getCell(i).getStringCellValue().trim());
			...
			row = sheet.getRow(dataStartRowNum + rowNum - 2);
			if (row == null)
	...
	public boolean setCellData(String sheetName, String colName, int rowNum, String data, String url) {
		...
		try {
			...
			if (colNum == -1)
				return false;
			sheet.autoSizeColumn(colNum); // vikrant
			row = sheet.getRow(rowNum - 1);
			if (row == null)
				row = sheet.createRow(rowNum - 1);
	...
	public static void main(String arg[]) throws IOException {
		...
		datatable = new Xls_Reader("C:\\Users\\e4814\\Desktop\\testdata.xlsx");
		// datatable.addColumn("Test", "USERNAME");
